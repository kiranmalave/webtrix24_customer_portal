...
define([
  ...
  'moment',
  'Swal',
  '../views/currencySingleView',
  '../collections/currencyCollection',
  '../models/currencyFilterOptionModel',
  '../../core/views/configureColumnsView',
  '../../core/views/mailView',
  'text!../templates/currencyRow.html',
  'text!../templates/currency_temp.html',
...
], function ($, _, Backbone, datepickerBT, moment, Swal, currencySingleView, currencyCollection, currencyFilterOptionModel, configureColumnsView, mailView, currencyRowTemp, currencyTemp, appSettings, dynamicFormData, moduleDefaultSettings, dynamicFilterView, linkedDropdown,timeselectOptions,deleteCardView) {
  var currencyView = Backbone.View.extend({
    ...
    initialize: function (options) {
      ...
      this.totalRec = 0;
      this.toClose = "currencyFilterView";
      this.arrangedColumnList = [];
      this.collection = new currencyCollection();
      ...
      this.skipFields = ['currency_icon'];
      this.staticJoined = [];
      this.idsToRemove = [];
      this.columnMappings = [{ 'price': 'Priced' }];    
      ...
      this.filterOption = new currencyFilterOptionModel();  
      this.filterOption.set({ "menuId": this.menuId });
      this.dynamicFormDatas = new dynamicFormData();
      this.collection.on('add', this.addOne, this);
      this.collection.on('reset', this.addAll, this);

      this.deleteURL = 'currencyMaster/multipleHardDelete';
	    this.statusChangeURL = 'currencyMaster/multiplecurrencyChangeStatus'; 
    ...
    changeStatusListElement: function (e) {
      if (permission.delete != "yes") {
        ...
        $('#currencyList input:checkbox').each(function () {
          if ($(this).is(":checked")) {
            removeIds.push($(this).attr("data-currency_id"));
          }
        });
        ...
        $.ajax({
          url: APIPATH + 'currencyMaster/status',
          method: 'POST',
    ...
    loadSubView: function (e) {
      ...
      switch (show) {
        ...
        case "singlecurrencyData": {
          var currency_id = $(e.currentTarget).attr("data-currency_id");
          ...
          if (currency_id != "" && currency_id != null && currency_id != undefined) {
            if (permission.edit != "yes") {
              ...
              new currencySingleView({ currency_id: currency_id, pluginId: selfobj.pluginId, searchcurrency: selfobj, menuId: selfobj.menuId, form_label: selfobj.form_label, menuName: this.menuName });
            }
          } else {
    ...
    addOne: function (objectModel) {
      ...
      var template = _.template(currencyRowTemp);
      ...
      $("#currencyList").append(template({ currencyDetails: objectModel, arrangedColumnList: this.arrangedColumnList }));
    },
    addAll: function () {
      $("#currencyList").empty();
 ...
 // SORT DATA ACCORDING TO COLUMN NAMES
  sortColumn: function (e) {
    ...
    render: function () {
      selfobj = this;
      var template = _.template(currencyTemp);
