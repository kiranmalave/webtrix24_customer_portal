...
], function ($, _, Backbone, validate, inputmask, datepickerBT, typeahead, icheck, select2, moment, Quill,Swal,customerCollection,invoiceItems,slugCollection,projectsCollection,menuCollection,singleTaxInvoiceModel,companySingleModel,taxInvoice_temp,additionalCharges_temp,invoiceNewRow,customerLookupRow,invoicePreview,shippingModalView,multiselectOptions,dynamicFieldRender,customerSingleView,readFilesView,categorySingleView,companySingleView) {
  var quotationToInvoiceView = Backbone.View.extend({
    ...
    attachEvents: function () {
      ...
      this.$el.off('click', '#ship_to', this.shipTocheck);
      this.$el.on('click', '#ship_to', this.shipTocheck.bind(this));
      ...
      this.$el.off('click', '.customerDetails-card', this.changeCustomer);
      this.$el.on('click', '.customerDetails-card', this.changeCustomer.bind(this));
      ...
      this.$el.off('click', '.addNewRecord', this.editCustomerDetails);
      this.$el.on('click', '.addNewRecord', this.editCustomerDetails.bind(this));
    ...
    addemptyRow: function (e) {
      ...
      $(".items-holder").append(template({categoryList : this.categoryList,slug_id : slug_id, row_number : (lastDetails+1) ,}));
      setTimeout(function () {
      ...
      if (name != "") {
        $.ajax({
          ...
          success: function (res) {
            ...
            if (res.msg === "sucess") {
              dropdownContainer.append('<div class="dropdown-item addNewRecord" data-view = "customer" style="background-color: #3F51B5; color:#ffffff;" > Add New Customer </div>');
              ...
              /*
                <div class="dropdown-item addNewRecord" data-view = "customer" style="background-color: #5D60A6; color:#ffffff;" >
                  Add New Customer
                </div>
              */
    ...
    updateTaxBox : function(){
      ...
      if (customer) {
        ...
        (customer.customer_image) ? $('#custProfile').attr('src', UPLOADS+'/customer/'+customer.customer_id+'/profilePic/'+customer.customer_image) : $('#custProfile').attr('src', UPLOADS+'/Default_cust.png');
    ...
    setProduct: function (e) {
      ...
      var product_description = $(e.currentTarget).attr('data-product_description');
    ...
    saveTaxInvoiceDetails: function (e) {
      ...
      selfobj.InheaderInfo.record_type = 'invoice';
      selfobj.InheaderInfo.inconvertToInvoice = 'yes' ;
    ...
    render: function () {
      ...
      $("#" + this.toClose).remove();
      this.$el.html(template({ model: this.model.attributes ,rows : this.rows, menuName : this.menuName,categoryList : selfobj.categoryList.models,companyDetails: selfobj.companyDetails ? selfobj.companyDetails[0] : [],projectsList:this.projectsList.models}));
