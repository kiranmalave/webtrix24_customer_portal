define([
  ...
  'text!../templates/deliveryReturnTemp.html',
  '../../customer/views/customerSingleView',
], function ($, _, Backbone, validate, inputmask, datepickerBT, typeahead, icheck, select2, moment, multiselectOptions, customerCollection, singleDeliveryChallanModel, dynamicFieldRender, invoiceItems, deliveryReturnTemp,customerSingleView) {

  var deliveryReturnView = Backbone.View.extend({
    ...
    initialize: function (options) {
      ...
      this.toClose = "deliveryReturnView";
    ...
    setnarration: function (product_id, last) {
      ...
      $.each(nlist, function (key, value) {
        if (product_id == value.product_id) {
          ...
          $("#itemName_" + last).val(value.productName+"/"+value.modelName+"/"+value.processor+"/"+value.generation+"/"+value.os+"/"+value.memory);
          ...
          success: function (res) {
            ...
            if (res.msg === "sucess") { 
              ...
              $.each(res.data, function (index, value) {
                dropdownContainer.append('<div id= "sd_'+lastDetails  +'" class="dropdown-item selectProduct" style="background-color: #ffffff;" data-productID=' + value.product_id + '>' + value.productName + '<span class="feature" >'+value.productName+"/"+value.modelName+"/"+value.processor+"/"+value.generation+"/"+value.os+"/"+value.memory+'</span></div>');
              });
