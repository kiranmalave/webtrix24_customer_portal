define([
  ...
  var campaingTemplateSelectorView = Backbone.View.extend({
    ...
    initialize: function (options) {
      ...
      this.columnlist = []; // ORIGINAL 
      this.parentObj.templateDetails = {};
      this.extractColumnList();
      this.columnList = this.columnlist;
      console.log(this.parentObj.moduleList);
    ...
    events:
    {
      ...
      "change .template_type": "templateTypeUpdate",
    ...
    attachEvents: function () {
      ...
      this.$el.off('change', '.template_type', this.templateTypeUpdate);
      this.$el.on('change', '.template_type', this.templateTypeUpdate.bind(this));
    ...
    extractColumnList:function() {
      var selfobj = this;
      var moduleList = this.parentObj.moduleList;
      Object.entries(moduleList).forEach(([key,module])=>{
        Object.entries(module).forEach(([modulekey,moduleval])=>{
          if (modulekey != 'conditionsView') {
            this.columnlist.push(...moduleval.columnlist);
          }
        });
      });
    },
    ...
    updateTemplateDetails: function () {
      var selfobj = this;
      if (this.parentObj.templateDetails['Email']) {
        var valuetxt = selfobj.parentObj.templateDetails['Email']['tempID'];
        console.log('valuetxt : ',valuetxt);
        console.log('selfobj.parentObj.templateDetails : ',selfobj.parentObj.templateDetails);
    ...
    getColumnList: function () {
      ...
      //   console.log('selfobj.filteredData : ',selfobj.filteredData);
      ...
      switch (toID) {
        case 'template_id':
          if (valuetxt == "addEmailTemp") {
            ...
            this.parentObj.templateDetails['Email']['tempID'] = valuetxt;
          }
          break;
        default:
          break;
    ...
    templateTypeUpdate: function (e) {
      ...
      if ($(e.currentTarget).is(':checked')) {
        this.parentObj.templateDetails[type] = {};
      }else{
        delete this.parentObj.templateDetails[type]; 
      }
      ...
      if (allchecked == true) {
        ...
        newdetails['template_type'] = 'both';
      } else {
        if (nottype.includes('both')) {
          nottype.pop('both');
        }
        $('#checkbox2').prop('checked', false);
        newdetails['template_type'] = nottype.join(',');
      } 
      console.log('this.templateDetails:" ',this.parentObj.templateDetails);      
      this.parentObj.model.set(newdetails);
    },
    // ATTACH QUILL 
    attachQuill: function () {
      ...
      selfobj.editor.on('text-change', function (delta, oldDelta, source) {
        if (source == 'api') {
          ...
          selfobj.parentObj.templateDetails['Email']['template_body'] =  justHtml;
          console.log(selfobj.parentObj.templateDetails);
          selfobj.emailTempModel.save({}, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded', 'SadminID': $.cookie('authid'), 'token': $.cookie('_bb_key'), 'Accept': 'application/json'
            }, error: selfobj.onErrorHandler, type: "POST"
      ...
      selfobj.whatsappEditor.on('text-change', function (delta, oldDelta, source) {
        if (source == 'api') {
          ...
          selfobj.emailTempModel.set({ "menuId": selfobj.emailMenuID });
          selfobj.emailTempModel.set({ "emailContent": justHtml });
          selfobj.parentObj.templateDetails['whatsapp'] = {'template_body' : justHtml};
          selfobj.emailTempModel.save({}, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded', 'SadminID': $.cookie('authid'), 'token': $.cookie('_bb_key'), 'Accept': 'application/json'
            }, error: selfobj.onErrorHandler, type: "POST"
      ...
      console.log('selfobj.templateDetails  : ',selfobj.parentObj.templateDetails);
    },
    // HANDLE ACCORDION
  ...
  return campaingTemplateSelectorView;
});
