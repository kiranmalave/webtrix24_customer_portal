...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	function verifyUserDetails($userName=null,$password=null)
	{
		if(!is_null($userName) && !is_null($password)){
			$this->db->where("(t.email='$userName' OR t.userName='$userName' OR t.contactNo='$userName') AND  t.status='active'");
	...
	public function setSessionKey($adminID=''){
		...
		$data = array("adminID"=>$adminID,"sessionKey"=>session_id(),"accessDate"=>$accessDate,"created_date"=>$accessDate,"IP"=>$ip);
		$res = $this->db->insert("admin_sessions",$data);
		$sqlerror = $this->db->error();
		$this->errorlogs->checkDBError($sqlerror,'SQL Error',dirname(__FILE__),__LINE__,__METHOD__);
		return $res;
	}
	public function unsetSessionKey($adminID=''){
		...
		$this->db->where($where);
		$res = $this->db->delete("admin_sessions");
		$sqlerror = $this->db->error();
		$this->errorlogs->checkDBError($sqlerror,'SQL Error',dirname(__FILE__),__LINE__,__METHOD__);
		return $res;
	}
	...
	public function updateSession($adminID=''){
		...
		$this->db->where("adminID",$adminID);
		$res = $this->db->update("admin_sessions",$data);
		$sqlerror = $this->db->error();
		$this->errorlogs->checkDBError($sqlerror,'SQL Error',dirname(__FILE__),__LINE__,__METHOD__);
	}
